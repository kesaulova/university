...
def viterbi_initialize(model, reference, read, k_max, viterbi_probability,  viterbi_backtracking):
    ...
    for i in range(1, len(read)):
        ...
        for k in range(1, k_max[i] + 1):
            ...
            if k == i and k <= length_first_hp(read):     # case V(i, 0, i, Insertion) - come from begining, i = len(firstHP)
                trans_prob = eln(model.HMM[0].transition('Begin', 'Insertion'))
                current_hp = hmm.homopolymer(read[i], k)
                emiss_prob = eln(model.HMM[0].emission(hmm.homopolymer(), current_hp, 'Insertion'))
            ...
            for k_prev in range(1, k_max[i - k] + 1):
                for state in states:
                    print "Previous, line 126 ", i - k, 0, k_prev, states[state], "transition ", eln(model.HMM[0].transition(states[state], 'Insertion')), \
                    "viterbi: ", viterbi_probability[i - k][0][k_prev][state]
                    # value = tran_prob * V(previous)
                    value = log_product(eln(model.HMM[0].transition(states[state], 'Insertion')),
...
def process_match(i, j, k, read, reference, k_max, viterbi_probability, viterbi_backtracking, model):
    ...
    print "---------------------------------- line 178",  i, j, k, states[0], 'Prob: ', viterbi_probability[i][j][k][0], read[i], reference[j].base, viterbi_backtracking[i][j][k][0]
    return 0
...
def process_insertion(i, j, k, read, reference, k_max, viterbi_probability, viterbi_backtracking, model):
    ...
    max_prob = float("-inf")
    prev_index = [0]*4
    emiss_prob = eln(model.HMM[j].emission(hmm.homopolymer(), hmm.homopolymer(read[i], k), 'Insertion'))
    value = float("-inf")
    ...
    if k_max[i - k] == 0:
        for state in states:
            ...
            "trans ", eln(model.HMM[j].transition(states[prev_index[3]], 'Insertion')), value
            #if (max_prob == 0 or value == 0) and max_prob != float("-inf"):
            #    if abs(value) > abs(max_prob):
            #        max_prob = value
    ...
    for k_prev in range(1, k_max[i - k] + 1):
        for state in states:
            ...
            eln(model.HMM[j].transition(states[prev_index[3]], 'Insertion')), value
    ...
    print "---------------------------------- line 232", i, j, k, states[2], viterbi_probability[i][j][k][2], viterbi_backtracking[i][j][k][2]
...
def process_deletion(i, j, k, viterbi_probability, viterbi_backtracking, model):
    ...
    value = float("-inf")
    for state in states:
        print 'Previous 234 line: ', i, j - 1, k, 'transition', eln(model.HMM[j - 1].transition(states[state], 'Deletion')), "viterbi", viterbi_probability[i][j - 1][k][state], states[state]
    ...
    print '---------------------------------- line 246', i, j, k, states[1], viterbi_probability[i][j][k][1], viterbi_backtracking[i][j][k][1]
    return 0
...
def viterbi_path(read, reference, model):
    """
    :param read: nucleotide sequence
    :param reference: homopolymer sequence
...
def parse_viterbi(probability, backtracking, reference_length, read):
    ...
    while backtracking[inf[0]][inf[1]][inf[2]][inf[3]] != 'Begin':
        print 'prob: ', probability[inf[0]][inf[1]][inf[2]]
