...
def generate_set(model, ref, set_size, len_seq, file_name):
    ...
    for i in range(set_size):
        gen_read, path = hmm.create_sequence(model, len_seq + 100, ref[:min(len_seq, len(ref))])
        result_set.append([gen_read, ref])
        f_set.writelines([gen_read, '\t', ref, '\n'])
    return result_set
...
def main(par_file_name, train_set_filename, cur_len, max_iterations):
    """
    ...
    while True:
        ...
        write_to_file_array(parameters_file, old_b)

        parameters_file.write("\n Current b: ")
        write_to_file_array(parameters_file, b)
        parameters_file.writelines(["Sigma: ", str(sigma)])
        parameters_file.close()
        ...
        print lik
        if abs(lik[i] - lik[i - 1]) < 0.15 or i > 12:
            break
        i += 1
...
for sample in range(50):
    tr_set_name = "./samples/" + str(sample) + ".txt"
    par_file = "./results/all_transitions_" + str(sample) + "_115_55.txt"
    main(par_file, tr_set_name, 115, 55)
